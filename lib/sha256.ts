...
export const ROTR = (value: number, amount: number) => (value >>> amount) | (value << (32 - amount))
...
export const sha256 = (text: string) => {
    ...
    for (let j: number = 0; j < words.length; j += 16) {
        ...
        for (let i: number = 0; i < 64; i++) {
            ...
            const word1: number = hash[7]
                + (ROTR(h4, 6) ^ ROTR(h4, 11) ^ ROTR(h4, 25))
                + ((h4 & hash[5]) ^ ((~h4) & hash[6]))
                + k[i]
                + w[i]

            const word2: number = (ROTR(h0, 2) ^ ROTR(h0, 13) ^ ROTR(h0, 22))
                + ((h0 & hash[1]) ^ (h0 & hash[2]) ^ (hash[1] & hash[2]))

            hash = [(word1 + word2) | 0].concat(hash)
            hash[4] = (hash[4] + word1) | 0
        }
        for (let i: number = 0; i < 8; i++) { hash[i] = (hash[i] + prevHash[i]) | 0 }
    }
